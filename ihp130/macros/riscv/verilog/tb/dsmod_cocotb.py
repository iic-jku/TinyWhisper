...
async def dsmod_psd_test(dut):
    ...
    out_path = os.path.join(os.path.dirname(__file__) or ".", "dsmod_output.txt")
    ...
    params_path = os.path.join(os.path.dirname(__file__) or ".", "dsmod_params.txt")
...
async def dsmod_sndr_single_test(dut):
    ...
    out_dir = os.environ.get("DSMOD_OUT_DIR", os.path.dirname(__file__) or ".")
...
async def dsmod_sndr_sweep_test(dut):
    ...
    out_path = os.path.join(os.path.dirname(__file__) or ".", "dsmod_sndr.txt")
    with open(out_path, "w") as f:
