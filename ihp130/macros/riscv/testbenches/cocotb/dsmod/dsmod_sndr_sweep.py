...
def _find_paths():
    ...
    src_path = os.path.normpath(os.path.join(script_dir, "..", "..", "..", "rtl"))
...
def main():
    ...
    result_dir = os.environ.get("DSMOD_RESULT_DIR", script_dir)
    build_dir = os.path.join(result_dir, "dsmod_sim_build")
    out_dir = os.path.join(result_dir, "dsmod_sndr_results")
    ...
    out_path = os.path.join(result_dir, "dsmod_sndr.txt")
    ...
    clk_freq = 56000000.0  # 56 MHz clock
    n_samples = 4096
    n_bitstream = n_samples * OSR
    df = clk_freq / n_bitstream
    sine_freq = round(8192 / df) * df
    params_path = os.path.join(result_dir, "dsmod_params.txt")
