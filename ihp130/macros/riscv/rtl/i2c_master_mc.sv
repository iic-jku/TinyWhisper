...
/*
 * Original Author: Chance Reimer
 * Adapted by Jonathan Hager to better fit the TinyWhisper project
 * Date: 4/12/2020
 **********************************************************************************************************************************************************
...
module i2c_master_mc (
    ...
    input next_chunk_ready,
    input read_next_byte,
    input force_stop,
    input use_subaddr,
    ...
    /** I2C Lines **/
    output logic scl_o,  //i2c clck line, output by this module, 400 kHz
    input  logic sda_i,
    output logic sda_o,
    output logic sda_oe,
  ...
  localparam [7:0] START_IND_SETUP = 20,  //Time before negedge of scl
  DATA_HOLD_TIME = 3,  //Time after negedge that scl is held
  STOP_IND_SETUP = 15;  //Time after posedge of scl before stop occurs
  ...
  always @(posedge i_clk) begin
    if (!reset_n) {clk_i2c_cntr, clk_i2c} <= 17'b1;
    else if (!en_scl) {clk_i2c_cntr, clk_i2c} <= 17'b1;
    else if (state == GRAB_DATA || state == READ_IDLE) begin
       // Nothing
      clk_i2c_cntr <= clk_i2c_cntr;
    end
    else begin
      clk_i2c_cntr <= clk_i2c_cntr + 1;
      if (clk_i2c_cntr == clock_division - 1) begin
        clk_i2c <= !clk_i2c;
        clk_i2c_cntr <= 0;
      end
    end
  ...
  always @(posedge i_clk) begin
    if (!reset_n) begin
      ...
      case (state)
        ...
        START: begin
          if(scl_prev & scl_curr & clk_i2c_cntr == START_IND_SETUP) begin   //check that scl is high, and that a necessary wait time is held
            ...
            if (use_subaddr) begin
               byte_sr <= {
               addr[7:1], 1'b0
               };  //Don't need to check read or write, will always have write in a read request as well
            end else begin
            byte_sr <= addr;
            end
            state <= SLAVE_ADDR;
        ...
        SLAVE_ADDR: begin
          ...
          if (byte_sent & cntr[0]) begin
            ...
            if (use_subaddr) begin
               ...
               next_state <= rw ? READ : WRITE;    //Check to see if sub addr was sent, we ony reach this state again if doing a read
        ...
        READ: begin
          if (byte_sent) begin
            ...
            next_state <= (num_byte_sent == byte_len - 1) ? STOP : READ_IDLE;  //Have we read all bytes?
        ...
        GRAB_DATA: begin
          if (grab_next_data) begin
            req_data_chunk <= 1'b1;
            grab_next_data <= 1'b0;
          end else if (next_chunk_ready == 1'b0) begin
            // Next chunk not ready yet. We idle and keep req == 1, so the memory module can use it to set
            // next_chunk_ready back to 0 once we've accepted the new chunk
            req_data_chunk <= 1'b1;
          end else begin
            state   <= WRITE;
          ...
          if (scl_is_low) begin  //negedge
            if (clk_i2c_cntr == DATA_HOLD_TIME) begin
              if (ack_in_prog) begin
                ...
                sda_oe <= next_state == STOP ? 1'b1 : 1'b0;
        ...
        /***
             ...
             */
        RELEASE_BUS: begin
          if (clk_i2c_cntr == clock_division - 3) begin
            en_scl <= 1'b0;
            state <= IDLE;
            // reg_sda_o <= 1'bz;
            sda_oe <= 1'b0;
      ...
      if (force_stop == 1'b1 && handled_force_stop == 1'b0) begin
         state <= STOP;
         handled_force_stop <= 1'b1;
      end
    end
  end
 ...
 */
  always @(negedge i_clk) begin
    if (!reset_n) begin
      ...
      sda_curr <= {sda_curr[0], sda_i};  //2 flip flop synchronization chain
      sda_prev <= sda_curr[1];
      scl_curr <= clk_i2c;
      scl_prev <= scl_curr;
    end
  ...
  assign scl_o = clk_i2c;  //the line will be pulled up to VCC so 1'bz is high

  assign byte_ready = state == (READ_IDLE) ? 1 : 0;
endmodule
