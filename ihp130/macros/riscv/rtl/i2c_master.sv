...
module i2c_master (
      ...
      case (state)
        ...
        RECV_FRAME: begin
          for (int i = 0; i < 32; i++) begin
            if ({frame_index, index} == i[4:0]) data_out[i] <= sda_i_sync1;
          end
        end
        STOP: state <= END;
        END: state <= END;
  ...
  always_ff @(posedge clk) begin
    if (command_trigger) begin
      case (state)
        ...
        RECV_FRAME_ACK: begin
          case (frame_index)
            2'd0: frame_acks[0] <= sda_i_sync1;
            2'd1: frame_acks[1] <= sda_i_sync1;
            2'd2: frame_acks[2] <= sda_i_sync1;
            2'd3: frame_acks[3] <= sda_i_sync1;
          endcase
        end
      endcase
  ...
  always_comb begin
    ...
    case (state)
      ...
      SEND_FRAME_ACK: sda_o = (has_next_index) ? 0 : 1;
