...
module dsmod #(
	...
	reg signed [NBIT-1+8:0]		data_step_r;

	// combinatorial step calculation (used only at sample boundaries)
	reg signed [NBIT-1+8:0]		data_step_calc_w;
	...
	assign o_ds   =	(i_mode == ORD1) ? ~accu1_r[NBIT+1+8] :
	...
	assign feedback1_w = {{2{1'b0}}, 1'b1, {(NBIT-1+8){1'b0}}};
	assign feedback2_w = {{4{1'b0}}, 1'b1, {(NBIT-1+8){1'b0}}};
	...
	// interpolate input data (linear ramp)
	// step size = (new_sample - old_sample) / OSR
	always @(*) begin
		case (i_osr)
			OSR32:	data_step_calc_w = (data_ext_w - data_pre_r) >>> $clog2(32);
			OSR64:	data_step_calc_w = (data_ext_w - data_pre_r) >>> $clog2(64);	
			OSR128:	data_step_calc_w = (data_ext_w - data_pre_r) >>> $clog2(128);
			OSR256:	data_step_calc_w = (data_ext_w - data_pre_r) >>> $clog2(256);
			default: data_step_calc_w = {(NBIT+8){1'bx}}; // undefined
		endcase
	end
  	...
  	always @(posedge i_clk) begin
		if (!i_rst_n) begin
			...
			if (i_ena_mod) begin
				...
				if (fetch_ctr_r == 8'd0) begin
					// new sample arrives: start interpolation from previous
					// sample and store current as new endpoint
					data_pre_r <= data_ext_w;
					data_interp_r <= data_pre_r;
					data_step_r <= data_step_calc_w;
				end else begin
					// interpolate input data (linear ramp)
					data_interp_r <= data_interp_r + data_step_r;
				end
			end
		end
	end
