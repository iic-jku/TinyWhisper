...
only_toplevel=true
value="
.include ../../verification/pex/iqmod_mixer_pex.spice
.param VDD=1.5
.param Vcm=VDD/2
.param temp=27
.param Cload=10p
...
set wr_vecnames
set wr_singlescale
wrdata $PROJECT_PATH/python/plot_simulations/data/iqmod_mixer_tb_tran.txt v(vinp1_IF) v(vinn1_IF) v(vinp2_IF) v(vinn2_IF) v(LO_I) v(LO_IX) v(LO_Q) v(LO_QX) v(vout_RF) v(vout_RF_filt)
...
* set wr_vecnames
* set wr_singlescale
* wrdata $PROJECT_PATH/python/plot_simulations/data/iqmod_mixer_tb_tran_fft.txt v(vout_RF_spec_db)
...
set wr_vecnames
set wr_singlescale
wrdata $PROJECT_PATH/python/plot_simulations/data/iqmod_mixer_tb_tran_fft_filt.txt v(vout_RF_filt_spec_db)
...
C {/foss/designs/TinyWhisper/ihp130/macros/coupled_resonator_lc_bpf/xschem/coupled_resonator_lc_bpf_1k_1k.sym} 1080 -880 0 0 {name=x3}
