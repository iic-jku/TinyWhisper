...
```sh
$ make macro FROM=iqmod NAME=mixer
...
2. `rtl/tinywhisper_core.sv`: instantiate the macro.
...
4. `flow/librelane/pdn_cfg.tcl`: a `define_pdn_grid -macro` block when the power pins of the macro are not on the layers of the default grid, see the `iqmod_top` block at the end of the file.
...
6. `schematic/xschem/tinywhisper.sch` or `tinywhisper_top.sch`: place the symbol of the macro where the chip instantiates it.
7. `testbenches/cocotb/tinywhisper_top_tb.py`: the `sources` list names the netlist or RTL and the stub of every macro.
8. The documentation: [doc/floorplan.md](../doc/floorplan.md), [doc/specifications.md](../doc/specifications.md), [doc/pinout.md](../doc/pinout.md) when pins change, and the chip [README.md](../README.md).

`.gitignore` and `REUSE.toml` need no line: their `ihp130/macros/*/` patterns already cover the build outputs and the files without a license header of every macro, so a new macro passes `reuse lint` and keeps its simulation outputs out of `git status` from the first build. Add the macro to git before running `reuse lint`: git does not report the ignored paths inside a folder it does not track yet, so the lint lists the build outputs of an untracked macro.
