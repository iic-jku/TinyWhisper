...
A `.subckt` port list cannot carry a name twice either, so a symbol pin is a net and not a pad. The chip has 7 VDD, 9 VSS, 3 IOVDD and 3 IOVSS pads on the four supply nets of the RTL, so each supply appears once on `tinywhisper_top.sym`, in the west group, and the remaining pads carry a plain net label in [tinywhisper_top.sch](schematic/xschem/tinywhisper_top.sch) instead of a port.
